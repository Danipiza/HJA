...
public class Player {
	...
	public void handValue(List<Card> hand) {
		...
		char paloRepetido = '_';
		 ...
		 int valoresConsecutivos = 0, valoresGutshot = 0, maxValoresConsecutivos = 0, maxValoresGutshot = 0;
		 Boolean space = false, gutshotEspecial = false;
		 int numRepeticiones = 0;
	        ...
	        for(int i = 0; i < hand.size() - 1; ++i) {
	            if(hand.get(i).getValue() == hand.get(i+1).getValue() + 1) {
	                ...
	                if (valoresConsecutivos == 3 && i == 3 + numRepeticiones && hand.get(0).getValue() == 14)
	                	gutshotEspecial = true;
	                valoresGutshot++;
	            ...
	            else numRepeticiones++;
	        }
	        
	        if (valoresConsecutivos == 3 && 3 + numRepeticiones == 4 && hand.get(0).getValue() == 14)
            	gutshotEspecial = true;
	        
	        if (hand.get(0).getValue() == 14) {
	        	if(hand.get(4).getValue() == 2) {
	        		if (valoresConsecutivos == 2) gutshotEspecial = true;
	            	valoresConsecutivos++;
	            	valoresGutshot++;
	            }
	            else if(hand.get(4).getValue() == 3) {
	                if(space) 
	                	valoresGutshot = valoresConsecutivos;
		...
		int auxDrawStraight = 0;
		if (maxPalos == 4 && drawFlush == false)
			drawFlush = true;

		if (gutshotEspecial)
			auxDrawStraight = 1; //gutshot
		else if(maxValoresConsecutivos == 3) 
			auxDrawStraight = 2; // open ended
		else if(maxValoresGutshot >= 3) 
			auxDrawStraight = 1; //gutshot

		if (auxDrawStraight > drawStraight) 
			drawStraight = auxDrawStraight;
	}
	...
	public void OmahaCardsValue() {
		//En este punto this.Cards contiene la 4 cartas de mano Omaha. Hacemos combinatoria.
		List<Card> hand = new ArrayList<Card>();
